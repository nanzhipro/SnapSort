...
final class OCRService: OCRServiceProtocol {
    ...
    func recognizeText(
        ...
        return try await recognizeText(
            from: image, cgImage: cgImage, preferredLanguages: preferredLanguages)
    }
    ...
    private func recognizeText(
        ...
        let languages =
            preferredLanguages.isEmpty ? configuration.preferredLanguages : preferredLanguages
        ...
        // VNRecognizeTextRequest的结果已经是[VNRecognizedTextObservation]类型
        let textObservations = observations

        // 对文本观察结果进行排序
        let sortedObservations = textObservations.sorted { (first, second) -> Bool in
